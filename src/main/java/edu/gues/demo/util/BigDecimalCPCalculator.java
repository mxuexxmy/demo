 ...
 * @description: TODO
 * @author: mxuexxmy
 * @date: 2023/12/1 23:00
 * @version: 1.0
 */
public class BigDecimalCPCalculator {
...
//        );
        ...
        // 调用方法计算 CP
        BigDecimal cp = calculateCP(measurements, usl, lsl, mathContext);

        System.out.println("CP: " + cp);
    }

    private static BigDecimal calculateCP(List<BigDecimal> measurements, BigDecimal usl, BigDecimal lsl, MathContext mathContext) {
        ...
        // 获取样本均值
        BigDecimal mean = new BigDecimal(stats.getMean(), mathContext).setScale(3, RoundingMode.HALF_EVEN);

        // 计算样本标准差
        BigDecimal stdDev = new BigDecimal(stats.getStandardDeviation(), mathContext).setScale(3, RoundingMode.HALF_EVEN);

        // 计算 CP
        BigDecimal six = new BigDecimal(6, mathContext);
        BigDecimal numerator = usl.subtract(lsl, mathContext);
        BigDecimal denominator = six.multiply(stdDev, mathContext);

        return numerator.divide(denominator, mathContext).setScale(3, RoundingMode.HALF_EVEN);
    }
